...
def load_llm():
    try:
        ...
        print("DEBUG: Using OpenAI GPT-4o")
        return llm
    except Exception as e:
        if "insufficient_quota" in str(e) or "quota" in str(e).lower():
            print("DEBUG: OpenAI quota exceeded, switching to Ollama")
            try:
                ...
                # Test the connection
                test_response = llm.complete("Test")
                print("DEBUG: Using Ollama llama3.2")
                return llm
            except Exception as ollama_error:
                print(f"DEBUG: Ollama also failed: {ollama_error}")
                raise e  # Re-raise the original OpenAI error
        else:
            raise e
